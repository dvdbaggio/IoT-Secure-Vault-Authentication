...
\begin{table*}[h]
    ...
    \midrule
    \texttt{VAULT\_SIZE} & 128 & Total keys in the Vault ($N$) \\
    \texttt{KEY\_SIZE\_BITS} & 128 & Bit-length of each Vault key \\
    \texttt{CHALLENGE\_INDICES} & 8 & Number of keys per challenge ($|C_1|, |C_2|$) \\
    \texttt{BLOCK\_SIZE} & 16 & AES block size in bytes (128 bits) \\
    \bottomrule
    \end{tabular}
...
The security and performance of the simulation are governed by the configuration constants defined in \texttt{config.py}. Table \ref{tab:params} summarizes the parameters used for this implementation.
